...
func bindRoutes(
	...
	r.Route("/apis/web/v1", func(r chi.Router) {
		...
		r.Group(func(r chi.Router) {
			...
			r.Get("/insights/genres/{genre}/tracks", handlers.GetGenreTracksHandler(db))
			r.Get("/insights/video-channels", handlers.GetTopVideoChannelsHandler(db))
			r.Get("/insights/video-format-split", handlers.GetVideoFormatSplitHandler(db))
			r.Get("/insights/video-activity", handlers.GetVideoActivityHandler(db))
